<template>
  ...
  <view v-else class="formContainer">
    <uni-forms
      ...
      :modelValue="turnipFormData"
      :border="true"
      label-width="160"
    >
      <uni-forms-item label="菜价" required name="price">
        <uni-easyinput
          v-model="turnipFormData.price"
          type="number"
      ...
      <uni-forms-item label="交易类型" name="exchangeType">
        <uni-data-checkbox
          ...
          v-model="turnipFormData.exchangeType"
      ...
      <uni-forms-item v-show="!isLineBL" label="联系方式" name="contact">
        <uni-data-checkbox
          selectedTextColor="#3FB984"
          selectedColor="#3FB984"
          v-model="turnipFormData.contact"
...
export default {
  ...
  setup(props) {
    ...
    const getTurnipInfo = async () => {
      isLoading.value = true
      const { data } = await getTurnip(itemId.value)
    ...
    const submitEdit = async () => {
      try {
        ...
        if (pickerTimeProps.validity.value) {
          turnipFormData.validTime = stringToStamp(
        ...
        const res = await addTurnip(turnipFormData)
        uni.showToast({
          title: res.message,
          duration: 2000,
          success: res => {
            goBack({ type: 1, getItemApi: getTurnip, id: itemId.value })
          },
        })
      } catch (err) {
        uni.showToast({
