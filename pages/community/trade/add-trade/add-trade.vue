<template>
  ...
  <view v-else class="formContainer">
    <uni-forms ref="tradeFormRef" :rules="tradeFormRules" :modelValue="tradeFormData"
      :border="true" label-width="160">
      <uni-forms-item label="上岛说明" name="detail">
        <uni-easyinput type="textarea" v-model="tradeFormData.detail" placeholder="门票/报酬等等,需要补充的内容都填这里"
          @input="isValid = true" />
      </uni-forms-item>
      <uni-forms-item label="交易类型" name="exchangeType">
        <!-- #ifndef MP-ALIPAY -->
        <picker mode="multiSelector" @columnchange="multiColumnChange" :value="multiIndex" :range="multiArray">
      ...
      <uni-forms-item v-show="isLineBL && isAutoBL" label="同时登岛人数限制">
        <uni-number-box v-model="tradeFormData.maxPeople" :min="1" :max="100"></uni-number-box>
      </uni-forms-item>
      <uni-forms-item v-show="isLineBL && isAutoBL" label="登岛超时时间(分钟)">
        <uni-number-box v-model="tradeFormData.maxTime" :min="1"></uni-number-box>
      </uni-forms-item>
      <uni-forms-item v-show="!isLineBL" label="联系方式" name="contact">
        <uni-data-checkbox selectedTextColor="#3FB984" selectedColor="#3FB984" v-model="tradeFormData.contact"
          :localdata="contactList">
        </uni-data-checkbox>
      </uni-forms-item>
      <uni-forms-item v-show="!isLineBL" :label="tradeFormData.contact" name="contactDetail">
        <uni-easyinput type="textarea" v-model="tradeFormData.contactDetail" placeholder="请输入具体联系方式"
          @input="isValid = true" />
      </uni-forms-item>
      <uni-forms-item v-show="isLineBL" label="开岛密码" name="psw">
        <uni-easyinput v-model="tradeFormData.psw" placeholder="请输入开岛密码" :inputBorder="false"
          @input="isValid = true"
...
<script>
  ...
  export default {
    setup(props) {
      ...
      const exchangeList = [{
          type: '交易',
          children: ['家具相关', '服饰相关', '材料相关', 'DIY手册相关', '活动相关', '花卉相关', '其他']
        },
        {
          type: '特殊NPC',
          children: ['傅珂(妹妹)', '骆岚(壁纸地板)', '薛革(鞋袜包包)', '绢儿(服装)', '狐利(艺术品)', '然然(园艺)', '其他']
        },
      ]
      ...
      const submitEdit = async () => {
        try {
          ...
          if (pickerTimeProps.validity.value) {
            tradeFormData.validTime = stringToStamp(pickerTimeProps.pickerTime.value)
          }
          ...
          tradeFormData.exchangeType = exchangeList[multiArr[0]].type + '-' + exchangeList[multiArr[0]].children[
            multiArr[1]]
