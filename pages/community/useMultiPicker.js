...
export default function useMultiPicker(optionList, formData) {
  ...
  */
  const multiIndex = computed(() => {
    ...
    if (formData.exchangeType) {
      ...
      indexArray[1] = optionList[indexArray[0]].children.findIndex(
        item => item === exchangeTypeArr[1]
      )
    }
    return indexArray
  })
  ...
  */
  const multiArray = computed(() => {
    return [firstArr, [...optionList[multiIndex.value[0]].children]]
  })
  ...
  const multiColumnChange = e => {
    ...
    if (e.detail.column === 0) {
      ...
      multiArray.value[1] = optionList[multiIndex.value[0]].children
    }
  }
