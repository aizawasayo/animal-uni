<template>
  ...
  <view v-else class="userContainer">
    ...
    <view class="userBtns radius-box">
      ...
      <view class="userBtn" @click="utils.goPage('/pages/user/settings/settings?id=' + userId)">
      ...
      <view class="userCard">
        ...
        <view class="cardInfoTop">
          ...
          <view class="cardInfoTopIsland">
            <uni-icons type="map-pin-ellipse" color="#1B5E20" size="16" class="cardIcon" />
            <text>{{userInfo.islandName}}</text>
            <uni-icons type="navigate-filled" color="#D81B60" size="16" class="cardIcon" />
            <text>{{userInfo.position === 'North' ? '北半球' : '南半球'}}</text>
          </view>
        </view>
        ...
        <view class="cardSideBtm">
          <view>登记日：
            <uni-dateformat :date="userInfo.startDate * 1000" format="yyyy/MM/dd"></uni-dateformat>
          </view>
          <text>{{`<<<<<<<<<`}}</text>
        </view>
      </view>
