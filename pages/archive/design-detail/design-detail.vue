<template>
  ...
  <view v-else-if="itemDetail !== null" class="detailContainer">
    <text class="detailTitle"> 【{{ typeName }}】{{ itemDetail.name }} </text>
    <view class="detailInfo">
      ...
      <view class="infoInline">
        ...
        <image
          class="userAvatar"
          v-if="itemDetail.user"
          :src="apiUrl + itemDetail.user.avatar"
        />
        <span v-if="itemDetail.user" style="flex: 1">{{
          itemDetail.user.nickname
        }}</span>
      </view>
    </view>
    <view
      class="uni-flex img-box"
      v-for="img in itemDetail.photoSrc"
      :key="img.uid"
    >
