  ...
  <view v-else class="padd-container">
    ...
    <view class="uni-list radius-box">
      ...
      <uni-list-item
        ...
        :rightText="
          $filters.textFilter(
            `${itemDetail.engName} , ${itemDetail.jpnName}`,
            30
          )
        "
      />
      ...
      />
      <uni-list-item
        v-if="itemDetail.type"
        :show-extra-icon="true"
        :extra-icon="{ type: 'list', color: '#ffb300' }"
        title="类型"
        :rightText="itemDetail.type"
      />
      <uni-list-item
        v-if="itemDetail.price"
        :show-extra-icon="true"
        :extra-icon="{ type: 'wallet', color: '#8BC34A' }"
        title="价格"
      ...
      <uni-list-item
        v-if="itemDetail.price"
        :show-extra-icon="true"
        :extra-icon="{ type: 'shop', color: '#0277BD' }"
        title="豆狸回收价格"
        :rightText="(itemDetail.price * 0.25).toString()"
      />
      <uni-list-item
        v-if="itemDetail.price"
        :show-extra-icon="true"
        :extra-icon="{ type: 'gift', color: '#00BB00' }"
        title="箱子回收价格"
        :rightText="(itemDetail.price * 0.2).toString()"
      />
      <uni-list-item
        v-if="itemDetail.size"
        :show-extra-icon="true"
        :extra-icon="{ type: 'paperclip', color: '#FF5809' }"
        title="占地尺寸"
        :rightText="itemDetail.size"
      />
      <uni-list-item
        v-if="itemDetail.colorInfo"
        :show-extra-icon="true"
        :extra-icon="{ type: 'color', color: '#F00078' }"
        title="颜色"
        :rightText="itemDetail.colorInfo"
      />
      <uni-list-item
        v-if="itemDetail.styleInfo"
        :show-extra-icon="true"
        :extra-icon="{ type: 'heart', color: '#00bcd4' }"
        title="风格"
        :rightText="itemDetail.styleInfo"
      />
      <uni-list-item
        v-if="itemDetail.themeInfo"
        :show-extra-icon="true"
        :extra-icon="{ type: 'vip', color: '#cddc39' }"
        title="主题"
        :rightText="itemDetail.themeInfo"
      />
      <uni-list-item
        v-if="itemDetail.series"
        :show-extra-icon="true"
        :extra-icon="{ type: 'eye', color: '#009688' }"
        title="所属系列"
        :rightText="itemDetail.series"
      />
      <uni-list-item
        v-if="itemDetail.orderType"
        :show-extra-icon="true"
        :extra-icon="{ type: 'cart', color: '#3f51b5' }"
        title="订购类型"
        :rightText="itemDetail.orderType"
      />
      <uni-list-item
        v-if="itemDetail.remould"
        :show-extra-icon="true"
        :extra-icon="{ type: 'starhalf', color: '#FDD835' }"
        title="改造类型"
        :rightText="itemDetail.remould"
      />
      <uni-list-item
        v-if="itemDetail.saleTime"
        :show-extra-icon="true"
        :extra-icon="{ type: 'calendar', color: '#1DE9B6' }"
        title="售卖时间"
        :rightText="itemDetail.saleTime"
      />
      <uni-list-item
        v-if="itemDetail.channelInfo"
        :show-extra-icon="true"
        :extra-icon="{ type: 'paperplane', color: '#ec407a' }"
        title="获取途径"
        :rightText="itemDetail.channelInfo"
      />
      <uni-list-item
        v-if="itemDetail.channelDetail"
        title="途径详情"
        :rightText="itemDetail.channelDetail"
      />
    </view>
      ...
      <view class="image-box">
        <view class="image" v-for="img in itemDetail.photoSrc">
          <image
            v-if="img.src"
            :src="apiUrl + img.src"
            @click.stop="utils.previewImage(apiUrl + img.src)"
            mode="aspectFit"
          />
          <image
            v-else
            :src="apiUrl + img"
            @click.stop="utils.previewImage(apiUrl + img)"
            mode="aspectFit"
          />
          <text v-if="img.src" class="imageText">{{
            $filters.itemColorFilter(img.name, itemDetail.name)
          }}</text>
        </view>
      </view>
    </view>
  ...
  setup(props) {
    ...
    const imageUrl = computed(() => {
      ...
      if (itemDetail.value && itemDetail.value.photoSrc.length !== 0) {
        val =
          typeof itemDetail.value.photoSrc[0] === 'string'
            ? `${apiUrl}${itemDetail.value.photoSrc[0]}`
    ...
    const getDetail = async id => {
      ...
      if (typeof data.type !== 'string')
        itemDetail.value.type = data.type.join('/')
