<template>
  <view class="comment-list">
    <uni-card :title="'最新评论('+total+')'">
      <scroll-view class="scroll-list" scroll-y @scrolltolower="loadMore"
        ...
        <view v-for="item in list" class="content-box">
          ...
          <view class="content-info">
            ...
            <view class="comment-footer">
              <uni-dateformat class="publish-time" :date="item.created_time*1000" :threshold="[60000, 3600000]">
              </uni-dateformat>
              <view v-if="userId ===item.uid._id" class="delete-text"
                @click="deleteById(item._id, () => deleteCallback(item._id))">删除</view>
...
<script>
  ...
  import { getComments, addComment, deleteComment } from '@/request_api/comment'
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const { deleteById } = useDelete(deleteComment, listProps.refreshList, type)
      ...
      const addLike = async (id, like) => {
        ...
        await addComment(type.value, comment)
        uni.showToast({
          title: '点赞成功！',
          success: res => {
            listProps.refreshList()
          }
        })
      }

      const deleteCallback = (cid) => {
        emit('fresh-comment')
        emit('delete-comment', cid)
      }
